...
module.exports = class Announce extends Command {
    ...
    async run(message) {
        ...
        message.author
            .send(new MessageEmbed().setTitle(`Where do you like to publish the announce? 🤔`).setDescription(`1) Same channel where I executed the command \n
          2) Another channel`))
            .then((botMessageSent) => {
            ...
            return message.author.dmChannel.awaitMessages((response) => response.length <= 200, { max: 1, time: 10000, errors: ["time"] });
        })
            .then((title) => {
