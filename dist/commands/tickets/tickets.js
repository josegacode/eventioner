    ...
    async run(message, { ticketId }) {
        getEventActiveInfo({
            ...
            getAttendeesTickets({
                ...
                if (attendeeFound) {
                    message.member.roles.add(message.guild.roles.cache.find((guildRole) => guildRole.name == "Participante"));
                    return message.embed(new MessageEmbed()
                        .setTitle(`Registro exitoso ✅`)
                        .setDescription(`@${message.author.username} tu boleto ha sido confirmado con éxito y tienes el rol de Participante,
                disfruta del evento! 🚀`)
                        .addField("\u200B", "\u200B")
