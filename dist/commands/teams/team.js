    ...
    async run(message) {
        ...
        message.author
	      ...
	      \> para que sean parte de tu equipo! ✅ 
            ...
            const filter = (choice) => parseInt(choice) > 0 && parseInt(choice) <= verticalsArray.length;
            return message.author.dmChannel.awaitMessages(filter, {
                max: 1,
                time: process.env.AWAIT_RESPONSE_TIMEOUT,
                errors: ["time"],
            ...
            // of find by name
            const teamBuildChannel = message.guild.channels.cache.find((guildChannel) => guildChannel.name === "arma-tu-equipo");
            return teamBuildChannel.send(new MessageEmbed()
                .setTitle(`
