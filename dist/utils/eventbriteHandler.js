...
const getAttendees = (async) => {
    return new Promise((resolve, reject) => {
        fetch(`${API_URL}events/${eventId}/attendees/?${OAuth}`).then((response) => resolve(response.json()));
    });
};
/**
...
const getAttendeesPage = async (pageNumber) => {
    return new Promise((resolve, reject) => {
        fetch(`${API_URL}events/${eventId}/attendees/?page=${pageNumber}&${OAuth}`).then((response) => resolve(response.json()));
    });
};
...
//# sourceMappingURL=eventbriteHandler.js.map