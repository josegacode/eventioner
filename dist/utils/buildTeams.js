    ...
    if (!reaction.message.embeds[0].title.includes("formando")) {
        ...
        if (!checkDuplicateTeamRequest(reaction)) {
            ...
            if (await teamIsReadyToBeBuilt(reaction)) {
                ...
                const teamRoleName = titleSpplited[titleSpplited.length - 3] +
                    " " +
                    titleSpplited[titleSpplited.length - 2];
                // Gets the team role object using
                // the role name extracted from the embed
                const teamRole = reaction.message.guild.roles.cache.find((guildRole) => guildRole.name === teamRoleName);
        ...
        if (await handleTeamBuild(reaction)) {
            ...
            // Save into db
            await insertNewTeam(teamInformation.role.name, event.event_id);
            ...
            await reaction.message.edit(new MessageEmbed()
                .setTitle(`
        ...
        `)
                ...
                .setTimestamp()
                .setFooter(process.env.FOOTER_MESSAGE)); // Invitation edition
        } // new team build
    } // new team case
};
module.exports = {
...
//# sourceMappingURL=buildTeams.js.map